...
def initialize(
    model_path: Union[str, None], device: Union[str, torch.device]
) -> Dict[str, Any]:
    if model_path is not None and os.path.exists(model_path):
        ...
        text_encoder_config = _default_text_encoder_dummy_config()
...
class T5_Func(Operator):
    ...
    # expose helpers and functional API on the class for backwards compatibility
    initialize = staticmethod(initialize)
    execute = staticmethod(execute)
    _default_text_encoder_dummy_config = staticmethod(
        _default_text_encoder_dummy_config
    )
    _default_tokenizer_path = staticmethod(_default_tokenizer_path)


if __name__ == "__main__":
    test_model_memory_allocation(model=T5_Func(), model_path=None)

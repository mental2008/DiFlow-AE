...
from diffusionflow.operators import T5_Func, Config


def create_workflow(model_path: str) -> Workflow:
    workflow = Workflow(name="t5_func_workflow")

    # Define model nodes
    t5_func = T5_Func(Config(model_path=model_path))

    # Define inputs
    prompt = workflow.add_input(name="text_prompt", data_type=str)

    # Define connections
    text_embed = t5_func(prompt=prompt)
...
if __name__ == "__main__":
    ...
    print(create_workflow(model_path=args.model_path))

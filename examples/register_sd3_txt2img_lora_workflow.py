# python examples/register_sd3_txt2img_lora_workflow.py
import argparse
...
def create_workflow(model_path: str, lora_model_path: str) -> Workflow:
    workflow = Workflow(name="sd3_txt2img_lora_workflow")
    ...
    lora = StableDiffusion3LoRA(Config(model_path=lora_model_path))
    ...
    # Add LoRA patches to the models
    clip_l.add_patch(lora)
    clip_g.add_patch(lora)
    sd3.add_patch(lora)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lora-model-path",
        type=str,
        default="/project/infattllm/lyangbk/huggingface/Yarn_art_SD3_LoRA",
    )
    ...
    service_id = register_workflow(
        workflow=create_workflow(
            model_path=args.model_path, lora_model_path=args.lora_model_path
        ),

...
def process_response(response, output_path, store_images):
    if response["status"] == "success":
        results = response["results"]
        if store_images:
            if isinstance(results["output_img"], list):
                assert len(results["output_img"]) == 1, "Only one image is supported for now"
                img = decode_image(results["output_img"][0])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-path", type=str, default="./output_img_0.png")
    ...
    if service_id == "flux_schnell_txt2img_workflow":
        ...
        process_response(response, args.output_path, args.store_images)
